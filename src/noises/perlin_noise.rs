...
impl PerlinNoise {
    ...
    pub(crate) fn get_value(&self, mut x: f64, mut y: f64, mut z: f64) -> f64 {
        ...
        return lerp(w, lerp(v, lerp(u, grad(self.p[aa], x, y, z),  // AND ADD
                                    grad(self.p[ba], x - 1.0, y, z)),// BLENDED
                            lerp(u, grad(self.p[ab], x, y - 1.0, z),  // RESULTS
                                 grad(self.p[bb], x - 1.0, y - 1.0, z))), // FROM  8
                    lerp(v, lerp(u, grad(self.p[aa + 1], x, y, z - 1.0),  // CORNERS
                                 grad(self.p[ba + 1], x - 1.0, y, z - 1.0)), // OF CUBE
                         lerp(u, grad(self.p[ab + 1], x, y - 1.0, z - 1.0),
